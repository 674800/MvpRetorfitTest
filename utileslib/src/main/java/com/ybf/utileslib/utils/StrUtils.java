...
public class StrUtils {
    private static String TAG = "StrUtils";
    // bt字节参考量
    ...
    // 获取ip地区
    public static String getIpCountry(Context mContext) {
    ...
    // 获取ip运营商
    public static String getIpName(Context mcontext) {
        String ipName = null;
        try {
            TelephonyManager mTelephonyManager = (TelephonyManager) mcontext.getSystemService(Context.TELEPHONY_SERVICE);
            if (mTelephonyManager != null) {
    ...
    // 获取ip基站
    public static String getIpBaseStation(Context mContext) {
        TelephonyManager telMgr = (TelephonyManager) mContext.getSystemService(Context.TELEPHONY_SERVICE);
        int cid = 0;
        int lac = 0;
        try {
            if (telMgr != null) {
                @SuppressLint("MissingPermission") GsmCellLocation gc = (GsmCellLocation) telMgr.getCellLocation();
                if (null == gc) {
                    return "0_0";
                }
                cid = gc.getCid();
            ...
            if (telMgr != null) {
                @SuppressLint("MissingPermission") CdmaCellLocation location = (CdmaCellLocation) telMgr.getCellLocation();
                if (null == location) {
                    return "0_0";
                }
    ...
    // 获取用户的IPd
    public static int getIpAddress(Context mContext) {
        int ipAddress = 0;
    ...
    // 获取用户设备IP
    public static String getUserIp(Context mContext) {
        int ipAddress = getIpAddress( mContext);
