package com.ybf.utileslib.crash;
...
import com.ybf.utileslib.utils.IoUtils;
import com.ybf.utileslib.utils.ManagerActivity;
import com.ybf.utileslib.utils.StrUtils;
import com.ybf.utileslib.utils.ThreadUtils;
 ...
 * Created by ybf on 2019/2/14.
 * 使用方法:
 * 在oncreat中调用CatchCrash.getInstance().setCustomCrashInfo(getApplicationContext());
 */
public class CatchCrash implements Thread.UncaughtExceptionHandler {
    ...
    private static final String CREAT_DIR="crash/"; //崩溃日志SD卡保存路径
    private static final String CARSH_LOG_DELIVER="http://img2.xxh.cc:8080/SalesWebTest/CrashDeliver";
    private  static Context mContext;
    private static CatchCrash instance = new CatchCrash();
    private CatchCrash() {
    }
    /**
     *
     ...
     *            异常信息对象
     */
    public   void saveToSdcard(Throwable ex) {
        String fileName = null;
        StringBuffer sBuffer = new StringBuffer();
        // 添加异常信息
        sBuffer.append(getExceptionInfo(ex));
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
            File file1 = new File(Environment.getExternalStorageDirectory().getAbsolutePath()+"/"+CREAT_DIR);
            Log.e(TAG,file1.getPath().toString());
            if (!file1.exists()) {
                file1.mkdir();
                Log.e(TAG,"mkdir");
            }
            fileName =  File.separator + paserTime(System.currentTimeMillis()) + ".log";
     ...
     * @param ex  崩溃异常
     */
    private void saveToServer(Throwable ex){
        final String carsh_log=getExceptionInfo(ex);
       // Log.e(TAG,"carsh_log");
        ThreadUtils.getInstance().runOnSubThread(new Runnable() {
            @Override
            public void run() {
